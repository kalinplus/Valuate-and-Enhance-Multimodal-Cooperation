...
class VideoDataset(Dataset):
    ...
    def __init__(self, directory, local_rank, num_local_rank, resize_shape=[168, 168], mode='val', clip_len=8,
        ...
        for file in tqdm(self.fnames, ncols=80):
            path_parts = Path(file).parts  # 自动适配不同系统的路径分隔符
            class_name = path_parts[-2]
            video_name = path_parts[-1]

            self.directory = Path('.') / 'pkl' / class_name
            pkl_file = self.directory / f"{video_name}.pkl"
    ...
    def loadvideo(self, capture, frame_count, fname):
        ...
        path_parts = Path(fname).parts
        self.directory = Path('.') / 'pkl' / path_parts[-2]
        pkl_file = self.directory / f"{path_parts[-1]}.pkl"

        if not self.directory.exists():
            self.directory.mkdir(parents=True, exist_ok=True)

        with open(pkl_file, 'wb') as Normal_writer:
...
if __name__ == '__main__':
    ...
    # datapath = '/code/data/UCF101'
    current_dir = Path(__file__).resolve().parent.parent
    datapath = current_dir / 'UCF-101'
    process_num = 4
